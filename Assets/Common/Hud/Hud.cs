...
{
    /// <summary>
    /// The on-screen read-out shared by every step: object count, frame time split, draw calls, memory,
    /// the frame-time strip, the flexibility card and a wall clock. Text refreshes at a fixed interval
    /// to keep string allocation off the per-frame path.
    /// </summary>
    public class Hud : MonoBehaviour
    {
        ...
        public bool Visible
        {
            ...
            set { if (_root != null) _root.style.display = value ? DisplayStyle.Flex : DisplayStyle.None; }
        }
        #endregion
        ...
        private Label _cardBehaviour, _cardEcosystem, _cardCost, _clock, _status;
        private VisualElement _barMain, _barRender, _barGpu;
        private FrameTimeStripHandle _strip;
        ...
        private void OnEnable()
        {
            ...
            QueryLabels();
            QueryBars();
            _strip = new FrameTimeStripHandle(_root.Q<FrameTimeStrip>("frame-strip"), _sampler);
            if (_switcher != null)
        ...
        /// <summary>Minimal mode shows only the status line, so benchmark runs render the same tiny overlay on every step.</summary>
        public void SetMinimal(bool minimal)
        {
            if (_root == null)
                return;
            foreach (var child in _root.Children())
                child.style.display = minimal && child != _status ? DisplayStyle.None : DisplayStyle.Flex;
        }
        #endregion
        ...
        #region Strip handle
        /// <summary>Binds the strip element to the sampler and repaints it; tolerates a missing element.</summary>
        private readonly struct FrameTimeStripHandle
        {
            private readonly FrameTimeStrip _strip;

            public FrameTimeStripHandle(FrameTimeStrip strip, FrameSampler sampler)
            {
                _strip = strip;
                if (_strip != null)
                    _strip.Sampler = sampler;
            }

            /// <summary>Requests a redraw of the strip for this frame.</summary>
            public void Repaint()
            {
